...
- NFC quick-log tags may carry only protocol version, the registered tag UUID, and whole uses `1..10`. The handler must require an exact local registry UUID/quantity match; no product, endpoint, date, label, event ID, or seconds may be placed on the tag. NFC pending-payload removal remains gated on durable Room persistence, and direct-uses Undo never deletes a Room row.
